...
This algorithm starts with the user providing an initial guess as to the best encoding filter and starts the search for a minimum near that filter. Only filters of the same length, $N$, as the initial filter will be tested. If any of the filters that are nearby within $N$-dimensional space, determined by the function `nearFilts', result in a smaller average bit size, determined by the user passed function `objFunc', then that filter will serve as the next location to search around for a minimum. This algorithm stores previously calculated average bit sizes in case the algorithm checks the same filter definition multiple times. Note that the function `nearFilts' automatically excludes any filters with $0$ on either end. This algorithm returns to the user the whole history of encoding filters tested and how small the average entry in the dataset was for each of those filters. It is possible for a simplistic algorithm such as this to fall into local minima but this can be mitigated by adjusting the size of the search parameter.

![Comparing the reduction in size vs the encoding time for a variety of filter lengths on the Nab dataset. While longer filters offered similar compressed sizes as delta encoding, the time taken to encode the data increased significantly with each additional parameter in the filter \label{fig:SizeVsEnc}.](SizeReductionAndEncodingTime.png){width="4in"}
